...
function __menu_editor_fill_icons(element)
{
    ...
    $.each(icons, function(key, value){
        $('<option value="'+value+'">'+escapeHtml('<i class="combo-icons '+value+'"></i> <span class="combo-icons-text">'+value+'</span>')+'</option>').appendTo($(element));
    });
}

function __menu_editor_start_icons()
{
    $('.mbicons').on('select2:opening', function (evt) {
        __menu_editor_fill_icons(this);
    });
}

function __menu_editor_update_label(element)
{
    $(element).parent().parent().find('label').contents()[1].textContent=' '+ $(element).val();
}

function __menu_editor_remove_item(element)
{
    $(element).closest('.linha').parent().remove();
}

function __menu_editor_get_name(element, child)
{
    if (child)
    {
        var name    = $(element).attr('name');
        var pedacos = name.split("_");
        var level   = pedacos[0];
        var name    = pedacos[1];
        
        level ++;
        
        return level+'_'+name+'_'+ Math.random();
    }
    else
    {
...
function __menu_editor_clone(linha, child)
{
    // remove selected color of tcolor on clone
    $(linha).find('.tcolor-icon').each(function(){
        $(this).css('background', 'unset');
    });

    // remove input values on clone
    $(linha).find('input').each(function(){
        var name = __menu_editor_get_name(this, child);
        
        // update name to show on post
        $(this).attr('name', name + Math.random());
        $(this).val('');

        // start auto complete for programs
        if (name.indexOf('action') != -1)
        {
            var new_id = 'mb' + (1 + Math.floor(Math.random() * 1000)) +''+ (1 + Math.floor(Math.random() * 1000));
            $(this).attr('id', new_id);
            setTimeout(function(){tentry_autocomplete( new_id, AdiantiMenuEditor.programs, '{}'); }, 300);
        }
        ...
        if (name.indexOf('color') != -1)
        {
            var new_id = 'mb' + (1 + Math.floor(Math.random() * 1000)) +''+ (1 + Math.floor(Math.random() * 1000));
            $(this).attr('id', new_id);
            setTimeout(function(){tcolor_start(new_id);}, 30);
        }
    });

    // remove waste of html and start a new tselec2
    $(linha).find('select').each(function(){
        var name = __menu_editor_get_name(this, child) + Math.random();
        ...
        var new_id = 'mb' + (1 + Math.floor(Math.random() * 1000)) +''+ (1 + Math.floor(Math.random() * 1000));
        $(this).attr('id', new_id);
        $(this).attr('data-select2-id', new_id);

        $(this).removeData('select2-id').removeAttr('data-select2-id').find('option').removeAttr('data-select2-id');
        $(linha).removeData('select2-id').removeAttr('data-select2-id');
        
        var parent = $(this).parent();
        $(parent).find('.select2-container').remove();

        setTimeout(function(){tcombo_enable_search('#'+new_id, 'Selecionar');}, 30);
        __menu_editor_fill_icons(this);
    });
...
function __menu_editor_add_sibling(element, direction)
{
    var $linha = $(element).closest('.linha').parent();
    var $irmao = $($linha).clone();
    $irmao.find('[sort-menu]').remove();
    
    $irmao = __menu_editor_clone($irmao, false);
    $($irmao).find('label').contents()[1].textContent=' '+ 'Label';
...
function __menu_editor_close_dropdown()
{
    $(".dropdown.show").removeClass("show");
    $(".dropdown-menu.show").removeClass("show");
}

function __menu_editor_add_child(element)
{
    var $linha = $(element).closest('.linha');
    var $child = $($linha).clone();
    var level  = $($linha).attr('level');
    level++;

    $($child).find('[class*="col-sm-"]').each(function(){
        $(this).attr('level', level);
    });
    
    $child = __menu_editor_clone($child, true);

    $child.attr('level', level);

    var left = 20 * (level - 1);

    var $label_child = $($child).find('label');
    $label_child.contents()[1].textContent=' '+ 'Label';
    $label_child.css('margin-left', left+'px');


    var $pai_linha = $linha.parent().find('.ui-sortable');
    
    if ($pai_linha.length > 0)
    {
        $pai_linha.append($child);
        $($child).wrap('<div level="'+level+'"></div');
    }
    else
    {
        $linha.after($child);
        $($child).wrap('<div sort-menu="order-menu" class="ui-sortable"></div>').wrap('<div level="'+level+'"></div');
    }
...
function __menu_editor_enable_sorting()
{
    $('[sort-menu=order-menu]').sortable({
        axis: 'y',
        cursor: 'move',
